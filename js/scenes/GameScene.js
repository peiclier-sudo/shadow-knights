...
export class GameScene extends Phaser.Scene {
    ...
    createBackground(width, height) {
        ...
        // Vanishing point (above screen, centered)
        const vpX = width * 0.5;
        ...
        // Perspective extent: how wide the floor is at a given Y
        const getFloorExtent = (y) => {
            const t = (y - vpY) / (floorBot - vpY);
            const halfW = width * 0.5 * t;
            return { left: vpX - halfW, right: vpX + halfW };
        };

        // Horizontal lines – quadratic spacing (bunched at top, spread at bottom)
        const numRows = 20;
        const rowYs = [];
        ...
        for (let i = 0; i < rowYs.length; i++) {
            ...
            const depthT = (y - floorTop) / (floorBot - floorTop);
            const alpha = 0.08 + 0.35 * depthT;
            grid.lineStyle(0.8 + depthT * 0.8, 0x2a2a4a, alpha);
            grid.beginPath();
        ...
        for (let i = 0; i <= numCols; i++) {
            ...
            grid.lineStyle(1, 0x2a2a4a, 0.25);
            grid.beginPath();
        ...
        this.backgroundDecor = { floor, grid };
        this._torches = [];
    }
